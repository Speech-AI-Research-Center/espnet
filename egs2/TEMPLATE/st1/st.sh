...
if ! "${skip_upload_hf}"; then
    if [ ${stage} -le 16 ] && [ ${stop_stage} -ge 16 ]; then
        ...
        [ -z "${***REMOVED***repo}" ] && \
            ...
            exit 1
        log "Stage 16: Upload model to HuggingFace: ${***REMOVED***repo}"
        ...
        ***REMOVED***repo_url="https://huggingface.co/${***REMOVED***repo}"
        if ! git ls-remote ${***REMOVED***repo_url} &> /dev/null; then
            log "Error: Repository '${***REMOVED***repo_url}' cannot be accessed. Please make sure you have access to it, or create one with e.g. huggingface-cli repo create ${***REMOVED***repo}."
            exit 1
        fi
        git clone ${***REMOVED***repo_url} ${dir_repo}
        ...
        # Generate description file
        # shellcheck disable=SC2034
        ***REMOVED***task=speech-translation
        # shellcheck disable=SC2034
        espnet_task=ST
        # shellcheck disable=SC2034
        task_exp=${st_exp}
