...
Options:
    ...
    --***REMOVED***repo        # Hugging face repository name (default="${***REMOVED***repo}").
...
if ! "${skip_upload_hf}"; then
    if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
        ...
        exit 1
        log "Stage 9: Upload model to HuggingFace: ${***REMOVED***repo}"
        ...
        ***REMOVED***task=self-supervised-learning
        # shellcheck disable=SC2034
        espnet_task=SSL
        # shellcheck disable=SC2034
        task_exp=${ssl_exp}
