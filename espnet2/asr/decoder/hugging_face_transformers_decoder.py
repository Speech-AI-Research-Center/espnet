...
class HuggingFaceTransformersDecoder(AbsDecoder, BatchScorerInterface):
    ...
    def score(self, ys, state, x, speech=None):
        ...
        # TODO caching
        model_inputs = self.***REMOVED***generate.prepare_inputs_for_generation(ys.unsqueeze(0), **model_kwargs)
        outputs = self.***REMOVED***generate(**model_inputs, return_dict=True, output_attentions=False, output_hidden_states=False)
    ...
    def batch_score(
        ...
        model_inputs = self.***REMOVED***generate.prepare_inputs_for_generation(ys, **model_kwargs)
        outputs = self.***REMOVED***generate(**model_inputs, return_dict=True, output_attentions=False, output_hidden_states=False)
        next_token_logits = outputs.logits[:, -1, :]
        next_token_scores = torch.nn.functional.log_softmax(
            next_token_logits, dim=-1
